...
class StringFinder extends AbsIdFinder {

    public StringFinder(Configuration configuration) {
        super(configuration);
    }

    @Override
    protected int getIdRes(Field field) {
        FindString findString = field.getAnnotation(FindString.class);
        return findString.id();
    }

    @Override
    protected void wireFromId(int id, Context context, Object object, Field field) {
        setField(field, object, context.getResources().getString(id));
    }

    @Override
    public Class<? extends Annotation> annotationClass() {
        return FindString.class;
    }
}
