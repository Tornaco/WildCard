...
public class Hooks implements Publishable {
    ...
    public static void publishTo(Scalpel scalpel) {
        new Hooks().publish(scalpel);
    }

    @Override
    public void publish(Scalpel scalpel) {
        new RequireRootWirer(scalpel.getConfiguration(), new ChrisRootRequester()).publish(scalpel);
    }
}
