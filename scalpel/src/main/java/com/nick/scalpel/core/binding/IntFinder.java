...
class IntFinder extends AbsIdFinder {

    public IntFinder(Configuration configuration) {
        super(configuration);
    }

    @Override
    protected int getIdRes(Field field) {
        FindInt findInt = field.getAnnotation(FindInt.class);
        return findInt.id();
    }

    @Override
    protected void wireFromId(int id, Context context, Object object, Field field) {
        setField(field, object, context.getResources().getInteger(id));
    }

    @Override
    public Class<? extends Annotation> annotationClass() {
        return FindInt.class;
    }
}
